...
PAPERSEARCH_TRAIN_BATCH_SIZE="$((PAPERSEARCH_GRPO_BASE_TRAIN_BATCH / AGENT_R1_GRPO_ROLLOUT_N))"
PAPERSEARCH_LOG_PROB_MICRO_BATCH="$((PAPERSEARCH_GRPO_BASE_LOG_PROB_MICRO_BATCH / AGENT_R1_GRPO_ROLLOUT_N))"
if [[ "$PAPERSEARCH_TRAIN_BATCH_SIZE" -lt 1 ]]; then
    echo "❌ PAPERSEARCH_GRPO_BASE_TRAIN_BATCH ($PAPERSEARCH_GRPO_BASE_TRAIN_BATCH) must be >= AGENT_R1_GRPO_ROLLOUT_N ($AGENT_R1_GRPO_ROLLOUT_N)." >&2
    exit 1
fi
